...
class SettingDef:
    ...
    min_value: int | None = (
        None  # integer settings only; None = no minimum; 0 = allow zero (unlimited); 1 = positive only
    )
...
SETTINGS_REGISTRY: dict[str, SettingDef] = {
    ...
    "CREDENTIALS_COLLECTION": SettingDef(
        ...
        requires=["METRICS_COLLECTION"],
        description=(
            "Collect credential-type usage counts every hour. Has no effect when METRICS_COLLECTION is disabled."
        ),
    ),
    ...
    "JOBEVENT_ROW_LIMIT": SettingDef(
        ...
        description=(
            ...
            "Set to 0 for no limit. Lower for memory-constrained installations."
        ),
    ),
    "JOBEVENT_JOB_LIMIT": SettingDef(
        ...
        description=(
            ...
            "Limits the SQL IN-clause size. Set to 0 for no limit."
        ),
    ),
    # ------------------------------------------------------------------ dashboard
    ...
    "HOURLY_RETENTION_DAYS": SettingDef(
        ...
        min_value=1,
        description=(
            "Days to keep HourlyMetricsCollection rows. Older rows are deleted by the daily cleanup_metrics_data task."
        ),
    ),
    "DAILY_RETENTION_DAYS": SettingDef(
        ...
        min_value=1,
        description=(
            "Days to keep DailyMetricsSummary rows. Older rows are deleted by the daily cleanup_metrics_data task."
        ),
    ),
    ...
    "TASK_RECORD_RETENTION_DAYS": SettingDef(
        ...
        default=5,
        min_value=1,
        description=("Days to keep completed and failed Task records. Recurring task templates are always preserved."),
    ),
    "DASHBOARD_TELEMETRY_RETENTION_DAYS": SettingDef(
        ...
        default=60,
        min_value=1,
        description=("Days to keep DashboardTelemetry rows generated by each dashboard collection run."),
    ),
    # ------------------------------------------------------------------ anonymization
