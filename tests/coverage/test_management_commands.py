...
def test_metrics_service_init_system_tasks():
    with patch("apps.tasks.tasks.create_system_tasks",
               return_value={"created": 5, "removed": 2, "tasks": ["Created: x"]}) as mock_create:
...
def test_metrics_service_tasks_cancel(user):
    ...
    task = Task.objects.create(name="cancel_this", function_name="hello_world", task_data={}, created_by=user, status="pending")
    out = StringIO()
    try:
...
def test_metrics_service_tasks_retry(user):
    ...
    task = Task.objects.create(
        name="retry_this", function_name="hello_world", task_data={}, created_by=user, status="failed", attempts=1, max_attempts=3
    )
    out = StringIO()
