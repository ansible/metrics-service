...
@pytest.mark.unit
def test_dashboard_read_permission_accepts_scoped_users_and_denies_no_role():
...
def test_dashboard_scope_maps_existing_organization_member_and_admin_roles():
    from django.contrib.auth import get_user_model

    from apps.core.models import Organization
    from apps.dashboard_reports.permissions import EDIT_DASHBOARD, VIEW_DASHBOARD, get_dashboard_scope

    user_model = get_user_model()
    member = user_model.objects.create_user(username="organization-member")
    admin = user_model.objects.create_user(username="organization-admin")
    member_org = Organization.objects.create(name="Member Org")
    admin_org = Organization.objects.create(name="Admin Org")

    def access_qs(request_user, permission):
        if request_user == member and permission == "member_organization":
            return Organization.objects.filter(pk=member_org.pk)
        if request_user == admin and permission in {"member_organization", "change_organization"}:
            return Organization.objects.filter(pk=admin_org.pk)
        return Organization.objects.none()

    with (
        patch("apps.dashboard_reports.permissions.has_super_permission", return_value=False),
        patch.object(Organization, "access_qs", side_effect=access_qs) as access_query,
        patch(
            "apps.dashboard_reports.permissions._organization_resource_map",
            return_value={member_org.pk: ORG_A_UUID, admin_org.pk: ORG_B_UUID},
        ),
    ):
        member_scope = get_dashboard_scope(member)
        admin_scope = get_dashboard_scope(admin)

    assert (VIEW_DASHBOARD, EDIT_DASHBOARD) == ("member_organization", "change_organization")
    assert [(org.id, org.can_edit) for org in member_scope.organizations] == [(member_org.pk, False)]
    assert [(org.id, org.can_edit) for org in admin_scope.organizations] == [(admin_org.pk, True)]
    assert [call.args[1] for call in access_query.call_args_list] == [
        VIEW_DASHBOARD,
        EDIT_DASHBOARD,
        VIEW_DASHBOARD,
        EDIT_DASHBOARD,
