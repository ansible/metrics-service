...
class TestMainURLsFileContent(TestCase):
    ...
    def test_urls_file_content(self):
        ...
        # Test that the file contains expected imports (matching platform-service-example)
        ...
        assert "from django.conf import settings" in content
        ...
        assert 'path("api/", include(api_urls))' in content
        assert 'path("", include(root_urls))' in content

        # Test that the file contains dynamic app loading
        assert "for app in settings.LOADED_APPS:" in content
        assert "importlib.import_module(url_module_path)" in content

        # Test that the file includes apps.urls for service-specific URLs
        assert 'path("", include("apps.urls"))' in content

        # Test that the file has the managed by framework warning
        assert "WARNING: DO NOT EDIT THIS FILE" in content
    ...
    def test_urls_file_docstring(self):
        ...
        assert '"""' in content
        assert "URL configuration for metrics_service" in content
