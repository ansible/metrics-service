...
# Load top level apps settings if exists
if (apps_settings := Path(apps_dir / "settings.py")).exists():
    DYNACONF.load_file(apps_settings, run_hooks=False)

# Developers can enable new apps via envvars
...
if apps_dir.exists():
    all_apps = [
        ...
        and Path(apps_dir / app.removeprefix("apps.").replace(".", "/")).exists()
    ]
    for app in all_apps:
...
if not DYNACONF.get("IS_RUNNING_TESTS") and DYNACONF.get("DEBUG"):
    DYNACONF.set("INSTALLED_APPS", "@merge_unique django_extensions")
